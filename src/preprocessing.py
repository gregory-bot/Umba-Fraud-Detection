...
def create_features(df, is_train=True, freq_encodings=None):
    ...
    df = pd.concat([df, card_type_dummies, channel_dummies], axis=1)
    ...
    if 'identity_rows' in df.columns:
        df['identity_rows'] = df['identity_rows'].fillna(0).astype(int)

    df['card_bank'] = df['card_bank'].fillna('unknown')
